  ...
  return (
    ...
    rounded-lg sm:w-[430px] '>
        <Link to={`/post/${post.slug}`} >
            <img src={post.image} alt={post.slug} className='h-[260px] 
            w-full object-cover group-hover:h-[200px] transition-all duration-300
            z-20'/>
        </Link>
        <div className='p-3 flex flex-col gap-3'> 
            ...
            <Link to={`/post/${post.slug}`} className='z-10 group-hover:bottom-0 
             absolute bottom-[-200px] left-0 right-0 border border-teal-500
